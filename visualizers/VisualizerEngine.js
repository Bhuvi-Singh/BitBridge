...
class AnimatedObject {
    constructor(id, type, props = {}) {
        ...
        Object.assign(this, {
            ...
            x: 0, y: 0, toX: 0, toY: 0,
            w: 50, h: 50, r: 20,
            text: '', fg: p.text, bg: p.nodeBg, border: p.nodeBorder,
            alpha: 1, highlighted: false, alwaysOnTop: false, visible: true,
            layer: 0, edgeTo: null, // for lines: {toId, thickness, color}
        }, props);
    }
    ...
    draw(ctx) {
        ...
        ctx.strokeStyle = this.highlighted ? ThemeColors.palette().highlight : this.border;
...
class ObjectManager {
    ...
    draw() {
        ...
        all.filter(o => !o.highlighted && !o.alwaysOnTop).forEach(o => o.draw(ctx));
        all.filter(o => o.highlighted && !o.alwaysOnTop).forEach(o => o.draw(ctx));
        all.filter(o => o.alwaysOnTop).forEach(o => o.draw(ctx));
    }

    get(id) { return this.objects.get(id); }
    snapshot() { return new Map([...this.objects].map(([k, v]) => [k, { ...v }])); }
    restore(snap) {
        this.objects = new Map([...snap].map(([k, v]) => [k, Object.assign(new AnimatedObject(v.id, v.type), v)]));
    }
    ...
    setPosition(id, x, y) { const o = this.get(id); if (o) { o.toX = x; o.toY = y; } }
    move(id, x, y) { this.setPosition(id, x, y); }
    setForegroundColor(id, color) { const o = this.get(id); if (o) o.fg = color; }
    setBackgroundColor(id, color) { const o = this.get(id); if (o) o.bg = color; }
    ...
    applyFinalPositions() {
        for (const o of this.objects.values()) { o.x = o.toX; o.y = o.toY; }
    }
    lerpStep(percent) {
...
class VisualizerEngine {
    ...
    _notify() { this.onFrameChange?.(this.stepIndex, this.steps.length - 1); }
}
...
class VisualizerRegistry {
    ...
    static list() { return [...this._map.keys()]; }
...
export { ThemeColors, AnimatedObject, ObjectManager, VisualizerEngine, VisualizerRegistry };