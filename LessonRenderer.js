...
function renderInline(text) {
	...
	out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, label, url) => {
		const safeUrl = /^https?:\/\//i.test(url) || url.startsWith('#') || url.startsWith('/')
			? url
			: '#';
...
export function renderMarkdown(source) {
	...
	while (i < lines.length) {
		...
		if (line.trim().startsWith('```')) {
			...
			i++;
			html.push(`<pre class="lesson-code-block"><code>${escapeHtml(codeLines.join('\n'))}</code></pre>`);
			continue;
		}
		...
		if (headerMatch) {
			...
			html.push(`<h${level} class="lesson-h${level}">${renderInline(headerMatch[2].trim())}</h${level}>`);
			i++;
			continue;
		}
		...
		html.push(`<p class="lesson-paragraph">${renderInline(paraLines.join(' '))}</p>`);
	}
...
export async function loadLesson(lessonId, container) {
	...
	if (!content) {
		container.innerHTML = '<p class="lesson-paragraph">Lesson content coming soon.</p>';
		return lesson;
	}
	...
	const hasSlideshow = content.type === 'slideshow' && buildSlideshowEmbed(content.slideshowUrl);
	const hasVisualizer = content.type === 'visualizer' && content.visualizerId;

	// Count how many mediums: if 2+ exist, show tabs
	const mediumCount = [hasVideo, hasMarkdown, hasSlideshow, hasVisualizer].filter(Boolean).length;
	...
	if (isHybrid) {
		...
		if (hasMarkdown) {
			...
			try {
				...
				console.warn(`Lesson markdown not found for "${lessonId}" at ${content.src}:`, err.message);
				articleContainer.innerHTML = '<p class="lesson-paragraph">Article content coming soon.</p>';
			}
		}
	...
	if (hasMarkdown) {
		container.innerHTML = html + '<p class="lesson-paragraph">Loading lesson…</p>';
		try {
			const res = await fetch(content.src);
			if (!res.ok) throw new Error(`${res.status}`);
			const text = await res.text();
	...
	container.innerHTML = html || '<p class="lesson-paragraph">Lesson content coming soon.</p>';
	return lesson;
}
...
export function initLessonTabs(container) {
	...
	tabBtns.forEach(btn => {
		btn.addEventListener('click', () => {
			...
			container.querySelector(`#lesson-tab-${tabName}`)?.classList.add('active');
		});
	});
}