...
const TOPS = [
  ...
  {
    id: "top-crimson-jacket",
    label: "Crimson Jacket",
    color: "#B83B3B",
    unlockXp: 100,
  },
  {
  ...
  {
    id: "top-forest-flannel",
    label: "Forest Flannel",
    color: "#2F5233",
    unlockXp: 200,
  },
  { id: "top-lab-coat", label: "Lab Coat", color: "#F1F1F1", unlockXp: 300 },
  {
    id: "top-graduation-robe",
    label: "Grad Robe",
    color: "#1a1a2e",
    unlockXp: 500,
  },
...
const CHARACTER_TABS = [
  ...
  { id: "top", label: "TOP", groups: [{ layer: "top", label: "Tops" }] },
  {
    id: "glasses",
    label: "GLASSES",
    groups: [{ layer: "glasses", label: "Glasses" }],
  },
  { id: "hat", label: "HATS", groups: [{ layer: "hat", label: "Hats" }] },
  {
    id: "jewelry",
    label: "JEWELRY",
    groups: [{ layer: "jewelry", label: "Jewelry" }],
  },
...
function renderHairLayers(id, hairHex) {
  ...
  switch (id) {
    ...
    case "hair-buzz":
      return {
        back: "",
        cap: `<ellipse cx="50" cy="29" rx="18.5" ry="19.5" fill="${hairHex}" opacity="0.55"/>`,
        detail: "",
      };
    case "hair-afro":
      return {
        back: "",
        cap: `<circle cx="50" cy="27" r="23" fill="${hairHex}"/>`,
        detail: "",
      };
    case "hair-curly":
      return {
    ...
    case "hair-bun":
      return {
        back: "",
        cap: capDefault,
        detail: `<circle cx="50" cy="8" r="5.5" fill="${hairHex}"/>`,
      };
    case "hair-mohawk":
      return {
        back: "",
        cap: "",
        detail: `<path d="M45 6 L55 6 L53 24 L47 24 Z" fill="${hairHex}"/>`,
      };
    case "hair-pigtails":
      return {
    ...
    case "hair-side-part":
      return {
        back: "",
        cap: capDefault,
        detail: `<path d="M40 12 Q44 18 42 24" stroke="${hairHex}" stroke-width="1.4" fill="none" stroke-linecap="round" opacity="0.6"/>`,
      };
    case "hair-short":
    default:
...
function buildAvatarSVG(character) {
  ...
  const shoulderRx =
    bodyShape.id === "body-slim" ? 24 : bodyShape.id === "body-broad" ? 32 : 28;
...
function renderAvatarDisplays(character) {
  ...
  document
    .querySelectorAll(
      ".avatar-mini, #profileAvatarDisplay, #modalAvatarPreview",
    )
    .forEach((el) => {
...
function migrateCharacter(oldChar) {
  ...
  const topMap = {
    "outfit-teal": "top-teal-tee",
    "outfit-orange": "top-orange-zip",
    "outfit-navy": "top-navy-hoodie",
    "outfit-purple": "top-violet-vest",
    "outfit-mono": "top-mono-black",
  };
  const migrated = { ...DEFAULT_CHARACTER };
  if (oldChar.body && skinMap[oldChar.body])
    migrated.skinTone = skinMap[oldChar.body];
  if (oldChar.outfit && topMap[oldChar.outfit])
    migrated.top = topMap[oldChar.outfit];
  if (oldChar.hair && HAIR_STYLES.some((h) => h.id === oldChar.hair))
    migrated.hairStyle = oldChar.hair;
...
const ALGO_INFO = {
  "bubble-sort": {
    ...
    about:
      "Bubble Sort is a simple comparison-based sorting algorithm that repeatedly compares adjacent elements and swaps them when they are in the wrong order. After each pass through the array, the largest unsorted element moves, or “bubbles,” to the end of the unsorted portion. The algorithm continues until a complete pass is made without any swaps. Bubble Sort is easy to understand and implement, but its O(n²) average and worst-case runtime makes it impractical for large datasets.",
    ...
    pseudocode: {
      english: [
        ...
        "  for each adjacent pair in the unsorted portion:",
        "    if the left element is greater than the right element:",
        "      swap the two elements",
        "      set swapped to true",
        "  reduce the unsorted portion by one element",
      ],
  ...
  bst: {
    ...
    about:
      "A Binary Search Tree (BST) is a hierarchical data structure in which every node follows an ordering rule: values smaller than a node are stored in its left subtree, while values larger than a node are stored in its right subtree. This ordering allows search, insertion, and deletion to follow a single path from the root rather than examining every node. When the tree is balanced, these operations take O(log n) time on average. However, if values are inserted in an unfavorable order, the tree can become skewed and degrade to O(n).",
  ...
  arraylist: {
    ...
    about:
      "An ArrayList is a resizable array that stores elements in a contiguous block of memory. Unlike a fixed-size array, an ArrayList can automatically grow when its capacity is reached. Accessing an element by index is very fast because the location can be calculated directly. Adding an element to the end is amortized O(1), while inserting or removing an element from the middle requires shifting subsequent elements and therefore takes O(n) time.",
    ...
    pseudocode: {
      english: [
        "to add a value at an index:",
        "  if the underlying array is full:",
        "    create a larger array",
        "    copy all existing elements into it",
        "  shift elements from the index onward one position to the right",
...
async function loadVisualizer(algorithmKey) {
  ...
  try {
    ...
    const { VisualizerRegistry } =
      await import("./visualizers/VisualizerEngine.js");
    ...
    document.getElementById("vizSkipBackBtn").onclick = () => {
      engine.reset();
      syncPlayPauseLabel();
    };
    document.getElementById("vizStepBackBtn").onclick = () => {
      engine.stepBackward();
      syncPlayPauseLabel();
    };
    ...
    document.getElementById("vizStepFwdBtn").onclick = () => {
      engine.stepForward();
      syncPlayPauseLabel();
    };
    document.getElementById("vizSkipFwdBtn").onclick = () => {
      engine.skipToEnd();
      syncPlayPauseLabel();
    };
...
function renderAvatarItemGrid() {
  ...
  if (!container) return;
  const tabConfig =
    CHARACTER_TABS.find((t) => t.id === activeCustomizeTab) ||
    CHARACTER_TABS[0];

  const groupsHtml = tabConfig.groups
    .map((group) => {
      const layer = group.layer;
      const selected = progress.character[layer];
      const itemsHtml = AVATAR_PARTS[layer]
        .map((part) => {
  ...
  container.innerHTML =
    groupsHtml +
    (avatarLockNotice
      ? `<div class="avatar-lock-notice">${avatarLockNotice}</div>`
      : "");
...
function isAllComplete(ids) {
  return (
    ids.length > 0 && ids.every((id) => progress.completedLessons.includes(id))
  );
}
...
function getTrackProgressPct(track) {
  ...
  const allIds = trackData.phases.flatMap((p) =>
    p.units.flatMap((u) => u.lessons.map((l) => l.id)),
  );
...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    category: "Streaks & Consistency",
    name: "On a Roll",
    desc: "Maintain a 3-day coding streak",
    icon: "🔥",
    xp: 50,
    check: (p) => p.streak >= 3,
  },
  ...
  },
  {
    ...
    category: "Streaks & Consistency",
    name: "Two-Week Titan",
    desc: "Maintain a 14-day coding streak",
    icon: "⚔️",
    xp: 200,
    check: (p) => p.streak >= 14,
  },
  ...
  },
  {
    ...
    category: "Streaks & Consistency",
    name: "Night Owl",
    desc: "Finish a lesson between 10 PM and 5 AM",
    icon: "🌙",
    xp: 30,
    check: (p) => !!p.achievementFlags?.time_night,
  },
  {
    id: "time_early",
    category: "Streaks & Consistency",
    name: "Early Bird",
    desc: "Finish a lesson between 5 AM and 8 AM",
    icon: "🌅",
    xp: 30,
    check: (p) => !!p.achievementFlags?.time_early,
  },
  {
    id: "time_weekend",
    category: "Streaks & Consistency",
    name: "Weekend Warrior",
    desc: "Complete 3 lessons over a single weekend",
    icon: "🛡️",
    xp: 75,
    check: (p) => !!p.achievementFlags?.time_weekend,
  },

  // --- AP CS A Mastery ---
  {
    id: "apcsa_p1",
    category: "AP CS A Mastery",
    name: "Primitive Pioneer",
    desc: "Complete Phase 1: Objects & Methods",
    icon: "📦",
    xp: 150,
    check: (p) => isAllComplete(getLessonIdsForPhase("ap-csa", 1)),
  },
  {
    id: "apcsa_p2",
    category: "AP CS A Mastery",
    name: "Logic Gatekeeper",
    desc: "Complete Phase 2: Selection & Iteration",
    icon: "🔀",
    xp: 150,
    check: (p) => isAllComplete(getLessonIdsForPhase("ap-csa", 2)),
  },
  {
    id: "apcsa_p3",
    category: "AP CS A Mastery",
    name: "Architect",
    desc: "Complete Phase 3: Class Creation",
    icon: "🏗️",
    xp: 200,
    check: (p) => isAllComplete(getLessonIdsForPhase("ap-csa", 3)),
  },
  {
    id: "apcsa_p4",
    category: "AP CS A Mastery",
    name: "Collection Master",
    desc: "Complete Phase 4: Arrays, ArrayLists & 2D Grids",
    icon: "📊",
    xp: 250,
    check: (p) => isAllComplete(getLessonIdsForPhase("ap-csa", 4)),
  },
  {
    id: "java_string",
    category: "AP CS A Mastery",
    name: "String Pooler",
    desc: "Complete all String method lessons",
    icon: "🧵",
    xp: 60,
    check: (p) => isAllComplete(getLessonIdsForUnit("ap-csa", 2)),
  },
  {
    id: "java_loop",
    category: "AP CS A Mastery",
    name: "Looping Legend",
    desc: "Finish all While, For, and Nested Loop lessons",
    icon: "🔁",
    xp: 75,
    check: (p) => isAllComplete(getLessonIdsForUnit("ap-csa", 5)),
  },
  {
    id: "java_recursion",
    category: "AP CS A Mastery",
    name: "Mirror Mirror",
    desc: "Complete both AP CS A recursion modules",
    icon: "🪞",
    xp: 100,
    check: (p) => isAllComplete(getLessonIdsForUnit("ap-csa", 12)),
  },
  {
    id: "apcsa_master",
    category: "AP CS A Mastery",
    name: "AP CS A Scholar",
    desc: "Finish 100% of the AP CS A Track",
    icon: "🎓",
    xp: 1000,
    check: (p) => getTrackProgressPct("ap-csa") >= 100,
  },
  ...
  // --- DSA Mastery ---
  {
    ...
    name: "Linear Leader",
    desc: "Complete Phase 1: Linear Structures & Big-O",
    icon: "📏",
    xp: 200,
    check: (p) => isAllComplete(getLessonIdsForPhase("dsa", 1)),
  },
  {
    id: "dsa_p2",
    category: "DSA Mastery",
    name: "Divide & Conquer",
    desc: "Complete Phase 2: Recursion & Sorting",
    icon: "⚔️",
    xp: 250,
    check: (p) => isAllComplete(getLessonIdsForPhase("dsa", 2)),
  },
  {
    id: "dsa_p3",
    category: "DSA Mastery",
    name: "Hierarchical Hero",
    desc: "Complete Phase 3: Trees, Heaps & Tries",
    icon: "🌳",
    xp: 300,
    check: (p) => isAllComplete(getLessonIdsForPhase("dsa", 3)),
  },
  {
    id: "dsa_p4",
    category: "DSA Mastery",
    name: "Graph Navigator",
    desc: "Complete Phase 4: Graphs & Dynamic Programming",
    icon: "🕸️",
    xp: 400,
    check: (p) => isAllComplete(getLessonIdsForPhase("dsa", 4)),
  },
  {
    id: "dsa_bigo",
    category: "DSA Mastery",
    name: "Asymptotic Scientist",
    desc: "Complete all Complexity Analysis lessons",
    icon: "⏱️",
    xp: 50,
    check: (p) => isAllComplete(getLessonIdsForUnit("dsa", 1)),
  },
  {
    id: "dsa_bits",
    category: "DSA Mastery",
    name: "Bit Manipulator",
    desc: "Master binary and low-level bit operations",
    icon: "⚙️",
    xp: 75,
    check: (p) => isAllComplete(getLessonIdsForUnit("dsa", 2)),
  },
  {
    id: "dsa_linked",
    category: "DSA Mastery",
    name: "Chain Breaker",
    desc: "Complete all Linked List mechanics",
    icon: "🔗",
    xp: 80,
    check: (p) => isAllComplete(getLessonIdsForUnit("dsa", 4)),
  },
  {
    id: "dsa_master",
    category: "DSA Mastery",
    name: "Algorithm Architect",
    desc: "Finish 100% of the DSA Track",
    icon: "🧠",
    xp: 1500,
    check: (p) => getTrackProgressPct("dsa") >= 100,
  },
  ...
  // --- Interactive Learning ---
  {
    ...
    desc: "Step completely through a Memory Stack animation",
    icon: "🔍",
    xp: 40,
    check: (p) => !!p.achievementFlags?.stackVizCompleted,
  },
  {
    id: "viz_tree",
    category: "Interactive Learning",
    name: "Tree Climber",
    desc: "Insert, delete, and traverse nodes in the BST widget",
    icon: "🌲",
    xp: 50,
    check: (p) => (p.achievementFlags?.bstInteractions || 0) >= 5,
  },
  {
    id: "viz_graph",
    category: "Interactive Learning",
    name: "Pathfinder",
    desc: "Execute both BFS and DFS visualizer traversals",
    icon: "🧭",
    xp: 60,
    check: (p) => !!(p.achievementFlags?.bfsRun && p.achievementFlags?.dfsRun),
  },
  {
    id: "viz_array",
    category: "Interactive Learning",
    name: "Index Slider",
    desc: "Perform a dynamic array shift in the array widget",
    icon: "↕️",
    xp: 30,
    check: (p) => !!p.achievementFlags?.arrayShiftExecuted,
  },
  {
    id: "viz_dp",
    category: "Interactive Learning",
    name: "Tabulator",
    desc: "Fill out a Dynamic Programming matrix manually",
    icon: "🔢",
    xp: 75,
    check: (p) => !!p.achievementFlags?.dpMatrixSolved,
  },
  ...
  // --- Quiz & Assessment ---
  {
    ...
    name: "Sharp Mind",
    desc: "Score 100% on any Progress Check",
    icon: "🎯",
    xp: 30,
    check: (p) => !!p.achievementFlags?.quiz_perfect_1,
  },
  {
    id: "quiz_streak_5",
    category: "Quiz & Assessment",
    name: "Flawless Streak",
    desc: "Score 100% on 5 Progress Checks in a row",
    icon: "💎",
    xp: 150,
    check: (p) => (p.achievementFlags?.consecutivePerfectQuizzes || 0) >= 5,
  },
  {
    id: "quiz_fast",
    category: "Quiz & Assessment",
    name: "Lightning Fast",
    desc: "Answer a Progress Check correctly in under 10s",
    icon: "⚡",
    xp: 40,
    check: (p) => !!p.achievementFlags?.quiz_fast,
  },
  {
    id: "quiz_comeback",
    category: "Quiz & Assessment",
    name: "Resilient Learner",
    desc: "Retry a failed Progress Check and score 100%",
    icon: "🔄",
    xp: 35,
    check: (p) => !!p.achievementFlags?.failedThenPassed,
  },
  ...
  // --- Level Milestones ---
  {
    ...
    icon: "⭐",
    xp: 100,
    check: (p) => calculateLevel(p.xp).level >= 5,
  },
  {
    id: "lvl_10",
    category: "Level Milestones",
    ...
    icon: "🌟",
    xp: 250,
    check: (p) => calculateLevel(p.xp).level >= 10,
  },
  {
    id: "lvl_25",
    category: "Level Milestones",
    ...
    icon: "👑",
    xp: 500,
    check: (p) => calculateLevel(p.xp).level >= 25,
  },
];
...
function loadProgress() {
  try {
    ...
    if (merged.unlockedAchievements === undefined)
...
function importProgressData(exportString) {
  ...
  if (exportString.startsWith(prefixV2)) {
    data = JSON.parse(
      decodeURIComponent(
        escape(atob(exportString.slice(prefixV2.length).trim())),
      ),
    );
  } else if (exportString.startsWith(prefixV1)) {
    data = JSON.parse(
      decodeURIComponent(
        escape(atob(exportString.slice(prefixV1.length).trim())),
      ),
    );
  } else {
  ...
  if (data.character)
    progress.character = { ...DEFAULT_CHARACTER, ...data.character };
  if (typeof data.streak === "number") progress.streak = data.streak;
  if (typeof data.streakFreezes === "number")
    progress.streakFreezes = data.streakFreezes;
  if (typeof data.currentLessonId === "string" || data.currentLessonId === null)
  ...
  if (Array.isArray(data.unlockedAchievements))
    progress.unlockedAchievements = data.unlockedAchievements;
...
document.getElementById("exportDataBtn")?.addEventListener("click", () => {
  window.prompt(
    "Copy your BitBridge export code (select all, then Ctrl/Cmd+C):",
    exportProgressData(),
  );
});
...
function render() {
  ...
  document.getElementById("userXpDisplay").textContent =
    `${currentLevelXp} / ${targetXp} XP to next level`;
  ...
  document.getElementById("sidebarUserLevel").textContent =
    `Level ${level} · ${progress.xp.toLocaleString()} XP`;
  document.getElementById("profileUserLevel").textContent =
    `Level ${level} · AP CS A Track`;

  document.getElementById("totalXpValue").textContent =
    progress.xp.toLocaleString();
  document.getElementById("dayStreakValue").textContent = progress.streak;
  document.getElementById("completedLessonsValue").textContent =
    progress.completedLessons.length;
  document.getElementById("streakFreezesValue").textContent =
    progress.streakFreezes;

  document.getElementById("profileTotalXp").textContent =
    progress.xp.toLocaleString();
...
function checkAndAwardAchievements() {
  ...
  ACHIEVEMENTS.forEach((ach) => {
    if (
      !progress.unlockedAchievements.includes(ach.id) &&
      ach.check(progress)
    ) {
...
function renderAchievements() {
  ...
  if (dashboardGrid) {
    const preview = [...ACHIEVEMENTS]
      .sort(
        (a, b) =>
          (progress.unlockedAchievements?.includes(a.id) ? 0 : 1) -
          (progress.unlockedAchievements?.includes(b.id) ? 0 : 1),
      )
      .slice(0, 6);
...
function populateCalendarMonthSelect() {
  ...
  let options = "";
  let y = CALENDAR_MIN.year,
    m = CALENDAR_MIN.month;
...
function renderCalendar() {
  ...
  const min = CALENDAR_MIN,
    max = getCalendarMax();
  if (prevBtn)
    prevBtn.disabled = year * 12 + month <= min.year * 12 + min.month;
  if (nextBtn)
    nextBtn.disabled = year * 12 + month >= max.year * 12 + max.month;
  ...
  ["S", "M", "T", "W", "T", "F", "S"].forEach(
    (d) => (html += `<div class="calendar-daylabel">${d}</div>`),
  );
    ...
    phase.units.forEach((unit) => {
      ...
      unit.lessons.forEach((lesson) => {
        ...
        const mediumsHTML = (lesson.mediums?.learn || [])
          .map(
            (code) =>
              `<span class="lesson-medium" data-medium="${code}">${getMediumLabel(code)}</span>`,
          )
          .join("");
        unitsHtml += `
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const { initializeVisualizers } =
    await import("./visualizers/visualizer-init.js");
  initializeVisualizers().catch((err) =>
  ...
  const usernameSelect = document.getElementById("usernameSelect");
  if (usernameSelect) {
    usernameSelect.innerHTML = NAME_BANK.map(
  ...
  document
    .getElementById("continueLessonBtn")
  ...
  document.getElementById("viewCourseMapBtn")?.addEventListener("click", () => {
    document
      ...
      .forEach((n) => n.classList.remove("active"));
    document
      .querySelector('.nav-item[data-view="view-course-map"]')
  ...
  document.addEventListener("click", (e) => {
    if (e.target.closest(".lesson-card[data-algo]")) {
      ...
      document
        ...
        .forEach((n) => n.classList.remove("active"));
      document
        .querySelector('.nav-item[data-view="view-visualizer"]')
        ?.classList.add("active");
      document
        .querySelectorAll(".view-content")
        .forEach((v) => v.classList.remove("active"));
  ...
  document
    ...
    ?.addEventListener(
      ...
      () => {
        if (!currentEngine)
          loadVisualizer(vizSelector ? vizSelector.value : "bubble-sort");
      },
      { once: true },
    );
  ...
  document
    .getElementById("calendarMonthSelect")
  ...
  document
    .getElementById("viewAllAchievementsBtn")
    ?.addEventListener("click", () => {
      document
        .querySelectorAll(".nav-item")
        .forEach((n) => n.classList.remove("active"));
      document
        .querySelector('.nav-item[data-view="view-profile"]')
        ?.classList.add("active");
      document
        .querySelectorAll(".view-content")
        .forEach((v) => v.classList.remove("active"));
      ...
      document
        .getElementById("allAchievementsSection")
